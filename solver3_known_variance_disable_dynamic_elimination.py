...
"""
solver for procedure 2, known variance, no dynamic elimination
"""
...
class Procedure:
    ...
    def __init__(self, blackbox, alpha=0.05, delta=0.5, std=np.sqrt(10), n0=10):
    ...
    def update_pi(self):
        # for i in self.set_candidate:
        for i in range(self.blackbox.n_sys):
            self.pi += -(self.sample_total[i] - self.sample_count[i] * self.mu_hat_prev[i]) ** 2 / self.std ** 2 / 2
    ...
    def update_ll(self, i):
        ...
        for j in range(self.blackbox.n_sys):
...
class Blackbox:
    def __init__(self, n_sys=20, sd=np.sqrt(10)):
        self.n_sys = n_sys
        self.mu_oracle = [1] + [0] * (self.n_sys - 1)
...
if __name__ == '__main__':
    ...
    for n_sys in [20, 50, 100]:
        bb = Blackbox(n_sys=n_sys)
        output = []
        for _ in tqdm.tqdm(range(num_iter)):
            solver = Procedure(bb, delta=delta, n0=10)
            # print(solver.mu_hat_curr)
            output.append(solver.run())

        print("")
        print("*" * 10)
        print("n_sys: {}, delta: {}".format(n_sys, delta))
        ...
        file = open(f"{n_sys}_{delta}.pkl", "wb")
